...
public class HC extends MDP {
  ...
  public void initialAction(MatrixD state,MatrixD action,Random random) throws MatrixException {
    action.set(MAction,-0.5);
    action.set(PAction,-0.5);
  }
  ...
  public void getAction(MatrixD state,MatrixD action,Random random) throws MatrixException {
    if ((action.val(MAction)==-0.5) && (action.val(PAction)==-0.5)) action.set(PAction,0.5);
    else if ((action.val(MAction)==-0.5) && (action.val(PAction)==0.5)) {action.set(MAction,0.5); action.set(PAction,-0.5);}
    else if ((action.val(MAction)==0.5) && (action.val(PAction)==-0.5)) action.set(PAction,0.5);
    else if ((action.val(MAction)==0.5) && (action.val(PAction)==0.5)) {action.set(MAction,-0.5); action.set(PAction,-0.5);}
  }
  ...
  public void randomAction(MatrixD state,MatrixD action,Random random) throws MatrixException {
  	if (random.nextDouble()>0.5) action.set(MAction,0.5); else action.set(MAction,-0.5);
  	if (random.nextDouble()>0.5) action.set(PAction,0.5); else action.set(PAction,-0.5);
  }

  /** Generates a random state from those possible.
    * @exception matrix.MatrixException Vector is wrong length. */
  public void randomState(MatrixD state,Random random) throws MatrixException {
    missileX.val = random.nextDouble();
    missileY.val = random.nextDouble();
    planeX.val = random.nextDouble();
    planeY.val = random.nextDouble();
  ...
  public double nextState(MatrixD state,MatrixD action,
    ...
    if(Math.sqrt(x*x+y*y)>2) { reinforcement=1; valueKnown.val=true;}
    if(Math.sqrt(x*x+y*y)<0.25) {reinforcement=-1; valueKnown.val=true;}
  ...
  //find Q value for action (-0.5,-0.5)
    action.set(MAction,-0.5); action.set(PAction,-0.5);
    f.evaluate();
    max1 = outputs.val(0);
  //find Q value for action (-0.5,0.5)
    action.set(PAction,0.5);
  ...
  //find Q value for action (-0.5,-0.5)
    ...
    switch (choice1) {
        case 1:  action.set(PAction,-0.5);
                 action.set(MAction,-0.5);
                 value = max1;
                 break;
        case 2:  action.set(PAction,0.5);
                 action.set(MAction,-0.5);
                 value = max2;
                 break;
        default: break;
    ...
    switch (choice2) {
        case 3:  action.set(PAction,-0.5);
                 action.set(MAction,0.5);
                 value = max3;
                 break;
        case 4:  action.set(PAction,0.5);
                 action.set(MAction,0.5);
                 value = max4;
                 break;
        default: break;
